...
```{r}
#save the boxplot
ggsave(boxplot , filename = "plots/boxplot_all_sessions_all_dogs.png", width = 10, height = 6, scale=0.8, dpi = 1200)
```
...
```{r}
#LL: add these to manuscript (SM), but first adapt to be consistent with above - data of all sessions pooled together
...
looking_away_catala <- data.frame(
      ...
      studies = "catala", bar_color="#665001",#"#59341F"
      total_trials = 382)
...
data.plot.vs.catala$pattern <- ifelse(data.plot.vs.catala$studies == "catala", "stripe", "none")
...
#striped version of each color is Catala et al. (2017) study
```
...
which(subj.data.test$pval.binom.fdr<.05)
 #after applying false discovery rate choice_binaryion, none of the individual performances is significantly above chance level
