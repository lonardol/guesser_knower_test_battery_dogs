...
```
###Time between sessions #LL: remove and remove column test date from data file
```{r}
date_data <- xdata %>%
...
```{r}
#save the boxplot
ggsave(boxplot , filename = "plots/boxplot_all_sessions_all_dogs.png", width = 10, height = 6, scale=0.7, dpi = 1200)
```
...
cor.test(cor_across_session$session_1, cor_across_session$session_2)
cor.test(cor_across_session$session_1, cor_across_session$session_3)
cor.test(cor_across_session$session_2, cor_across_session$session_3)
...
plot_data_pretty <- plot_data %>%
  mutate(
    ...
    condition = factor(
      ...
      levels = c(
        "overall",
        sort(setdiff(unique(condition), "overall"))
      )
    )
  )
...
#Save test-retest plot
  1
```{r}
ggsave(test_retest_p, filename = "plots/density_plot_test_re-test_rel.png", width = 12, height = 15, scale = 0.6, bg = "white", dpi=1200) 
```
...
side.pref.model.data <- xdata %>%
  filter(condition!="present") %>%
  droplevels() %>% 
  group_by(dog_name) %>% #LL: and session?
  mutate(
...
```
#### switches between cups
```{r}
#LL: create left-right cup chosen varibale and re-do the above analyses
```
